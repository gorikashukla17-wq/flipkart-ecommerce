...
public final class Database {
    ...
    /**
     * Reads SHOPKART_DB_URL / SHOPKART_DB_USER / SHOPKART_DB_PASSWORD, falling back
     * to an in-memory Derby database when no URL is configured.
     */
    public static Database fromEnvironment() {
        String url = System.getenv("SHOPKART_DB_URL");
        if (url == null || url.isBlank()) {
            return inMemory("shopkart");
        }
        return new Database(url, System.getenv("SHOPKART_DB_USER"), System.getenv("SHOPKART_DB_PASSWORD"));
    }
